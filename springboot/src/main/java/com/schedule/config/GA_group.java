...
public class GA_group {
    ...
    private int len =6;//染色体长度，需要传入上班前工作时间，下班后工作时间
    ...
    private boolean fitness(int PreNum,int FreeNum,int AftNum,ArrayList<Double> PassFlowNum,ArrayList<ArrayList<Integer>> loves_arr) {
        //如果现在len是28，代表 pre 24 later的一个时间构造，解析上班时间的时候，就需要传上下班这个参数来确定
        //修改后的基因序列应该为 6 保证前四个基因序列不连续即可，对最后结果进行检查 如果连续对基因进行二次变异
        Random r = new Random();
...
//        for(int i=pre;i<len;i++) {
        for(int i=0;i<len;i++) {
            ...
            if (i < len) PopulationsNeed = PassFlowNum.get(i);
...
//                }//今天就工作完了
            ...
            //首先是确定需要的人数
            if(countPopulations(i)<=(int)Math.ceil(PopulationsNeed)){//还需要人上班，可以上的都上去
                for(int j=0;j<Size;j++)
                {//循环每一个
                    if(persistentTime(j)>0&&persistentTime(j)<8&&group.get(j).chromo.get(i)==1&&status.get(j).get(i)==1&&(i==0||status.get(j).get(i-1)!=2)){//等待上班，并且可以上
                ...
                while((int)Math.ceil(PopulationsNeed)!=countPopulations(i)) {//人不够,随机选在wait的人上班,每次选一个
                    ...
                    int index = Math.abs(r.nextInt()%Size);
                    if(check.get(index).get(i)==0&&status.get(index).get(i)==1&&DayTime(index)<16&&(i==0||status.get(index).get(i-1)!=2))
                    {//满足要求，上班
            ...
            //决定执行工作时长<2时就算人太多也不下班
                    ...
                    for(int j=0;j<Size;j++)
                    {
                        if(group.get(j).chromo.get(i-1)==1&&persistentTime(j)<4&&persistentTime(j)>0&&(i==0||status.get(j).get(i-1)!=2))
                        {
                            Chromo work = group.get(j);
                            work.chromo.set(i, 1);
                            group.set(j, work);
                    ...
                    while(countPopulations(i)<(int)Math.ceil(PopulationsNeed)) {
                        ...
                        int index = Math.abs(r.nextInt()%Size);
                        if(check.get(index).get(i)==0&&status.get(index).get(i)==1&&DayTime(index)<16&&persistentTime(index)<8&&(i==0||status.get(index).get(i-1)!=2)) {
...
//初始化员工状态，对于本需求工作时间需要修改
    ...
    //GA算法主体
    public ArrayList<Chromo> GA(int PreNum, int FreeNum, int AftNum, ArrayList<Double> PassFlowNum,int day,ArrayList<ArrayList<Integer>> loves_arr)//偏好数组和星期几，用于check
    {
...
//                    for(int i=0;i<6;i++){//代表六个时段，这里只区分时段
//                        //9:00-21:00 或者 10:00-22:00
...
//        }
        ...
        while(fitness(PreNum,FreeNum,AftNum,PassFlowNum,loves_arr)!=true)//while循环中一直在进行种群更新
        {
            //在fitness中进行变异
        }
